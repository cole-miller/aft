...
struct raft_message {
	...
	union raft_message_contents {
		struct raft_call_contents {
			...
			raft_index commit; // n/a for RAFT_VOTE_CALL
			uint16_t num_entries; // n/a for RAFT_VOTE_CALL
			struct raft_entry entries[RAFT_MAX_APPEND_ENTRIES]; // n/a for RAFT_VOTE_CALL
		} call_contents;
