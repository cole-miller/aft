...
static void raft_print_message(
	FILE *f, raft_server_id dest, const struct raft_message *msg)
{
	...
	case RAFT_APPEND_CALL:
		fprintf(f, "index=%" SCNu32 " num_entries=%" SCNu16,
			msg->contents.call_contents.index, msg->contents.call_contents.num_entries);
		break;
	case RAFT_VOTE_RESULT:
	case RAFT_APPEND_RESULT:
		fprintf(
			f, "result=%s", msg->contents.result_contents.result ? "SUCCESS" : "FAILURE");
		break;
	default:
		break;
	}
 ...
 */
static void raft_send_message(
	const struct raft_context *ctx, raft_server_id dest, const struct raft_message *msg)
{
	...
	if (sendto(ctx->server_sock, buf, RAFT_MAX_MESSAGE_SIZE, 0,
			(struct sockaddr *)&(ctx->server_addrs[dest]),
 ...
 */
static void raft_respond_vote(
	const struct raft_context *ctx, raft_server_id dest, raft_bool result)
{
...
static void raft_respond_append(
	const struct raft_context *ctx, const struct raft_message *req, raft_bool result)
{
	struct raft_message msg = {.type = RAFT_APPEND_RESULT,
		.sender = ctx->my_id,
		.sender_term = persist->current_term,
		.contents = {.result_contents = {.result = result,
...
static void raft_request_append(const struct raft_context *ctx,
	const struct raft_transient *trans, raft_server_id dest, uint16_t num_entries,
	raft_index start)
{
	assert(start > 0);
	struct raft_message msg = {.type = RAFT_APPEND_CALL,
		.sender = ctx->my_id,
		.sender_term = persist->current_term,
...
static void raft_request_all_appends(const struct raft_context *ctx,
	const struct raft_transient *trans, const raft_index *next_indices)
{
	raft_server_id i;
...
static void raft_follower(
	struct raft_context *ctx, int client_fd, int persist_fd, struct raft_transient *trans)
{
	struct pollfd poll_fds[2] = {
		{.fd = client_fd, .events = POLLIN}, {.fd = ctx->server_sock, .events = POLLIN}};
	...
	while (true) {
		...
		if (poll_fds[1].revents & POLLIN) {
			...
			// upon seeing a message with a newer term. If we're a follower
			// already, we might as well continue to process the current message
			// after updating our term.
			...
			if (msg.type == RAFT_VOTE_CALL) {
				...
				// Check whether the candidate's log is at least as up to date
				// as ours, to prevent an "ignorant" server that's missing
				// entries that are present on a majority of servers from
				// becoming leader and erasing these entries.
				if (msg.sender_term == persist->current_term &&
				...
				if (msg.sender_term == persist->current_term) {
					if (contents->index <= persist->last_index &&
						...
						if (RAFT_PERSIST_SIZE_UPTO(contents->index + 1 +
								contents->num_entries) > persist->max_size) {
						...
						// Update our view of what's committed, and apply any
					...
					// Reset the timer whenever we see an append request from
					// the current term, even if it was unsuccessful, since this
					// means we're still in touch with the leader.
					raft_reset_timer();
				} else {
					raft_sync();
					raft_respond_append(ctx, &msg, RAFT_FAILURE);
				...
				// Followers don't send call messages, so any result message we
				// receive must be left over from a previous term.
				raft_restore_timer(&saved);
			}
		}
	}
	...
	struct raft_context *ctx, int client_fd, int persist_fd, struct raft_transient *trans)
{
	...
	struct raft_message req = {.type = RAFT_VOTE_CALL,
		.sender = ctx->my_id,
		.sender_term = persist->current_term,
		.contents = {.call_contents = {.index = persist->last_index,
	...
	while (true) {
		...
		if (poll_fds[1].revents & POLLIN) {
			raft_receive_message(ctx, &msg);
			raft_pause_timer(&saved); // (candidates never reset their timers)
			...
			// resent and we can process it from follower state. This way, we
			// don't have to replicate the full follower logic in this function.
			if (msg.sender_term > persist->current_term) {
				persist->current_term = msg.sender_term;
				raft_sync();
				persist->voted_for = 0;
	...
	struct raft_context *ctx, int client_fd, int persist_fd, struct raft_transient *trans)
{
	...
	for (i = 1; i <= ctx->num_servers; ++i) {
		...
		// any entries that aren't known to be replicated on a majority of
		// servers.
		matched_indices[i] = 0;
	}
		...
		if (poll_fds[1].revents & POLLIN) {
			...
			if (msg.type == RAFT_VOTE_CALL) {
				...
				if (contents->result == RAFT_SUCCESS) {
					...
					// The MAXes here are to handle the possibility that this
					// response corresponds to entries that we've already marked
					// as replicated on the sending server (possible due to
					// reordering and resending).
					...
					// Count down from the newest matched index to the
					// known-committed index, looking for entries that have
					// achieved majority replication. This loop may degenerate
					// (correctly) if the response didn't indicate replication
					// of anything that isn't already committed. Note how we
					// only apply this treatment to entries from our current
					// term--this is required for correctness (see the paper for
					// details). Entries from previous terms will be committed
					// as soon as any entry from the current term is committed,
					// including the no-op from before.
					for (j = matched_indices[msg.sender]; j > old_commit &&
						...
						if (2 * replicas > ctx->num_servers) {
							// Majority replication achieved for this entry and
							// hence for all entries preceding it in the log.
							// (This is why we count down in the loop above.)
							trans->commit_index = j;
							// Apply the newly committed entries to our state
							// machine. Also alert our client for entries from
							// the current term.
							for (jj = old_commit + 1; jj <= j; ++jj) {
								...
								if (persist->log[jj].type == RAFT_ENTRY_NORMAL &&
									persist->log[jj].term_added ==
										persist->current_term) {
									raft_alert_client(
										client_fd, RAFT_COMMAND_APPLIED, NULL);
								}
							}
							trans->applied_index = j;

							// Now we've committed an entry from our term, and
							// in particular we've committed the special no-op
							// entry.
							if (!committed_noop) {
								committed_noop = true;
								if (reader_waiting && 2 * num_heard > ctx->num_servers) {
									...
									raft_alert_client(
										client_fd, RAFT_READ_RESULT, read_res);
					...
					// An append request failed. Again, we may have already
					// handled a logically "later" response, so instead of just
					// decrementing the next index for the sending server we
					// examine the index stated in the response itself, using
					// this only if it would actually decrease the next index.
					assert(contents->index > 0);
					next_indices[msg.sender] =
						MIN(contents->index, next_indices[msg.sender]);
				}

				// In any case, if we receive an append result from the current
				// term we've heard from the sending server.
 ...
 * Setup routines.
 */
static void raft_init_context(
	raft_server_id my_id, FILE *config, struct raft_context *ctx)
{
	uint16_t port;
	...
	const char *persist_path, int client_fd)
{
	...
	if (mode == RAFT_NORMAL_MODE) {
		...
		// The first log entry is a dummy entry--it's there to ensure that the
		// logs of two servers always have a nonempty common prefix. Here it's
		// tagged as a no-op to prevent the server from trying to apply it to
		// the state machine.
		persist->log[0].type = RAFT_ENTRY_NOOP;
		persist->log[0].term_added = 0;
	} else {
		// Check whether the log we're recovering from was expanded, and if so
		// remap the memory appropriately.
