...
static raft_server_response_type
request_read(int server_fd, const struct read_request *req, struct read_result *res)
{
    ...
    while (true) {
        ...
        while (true) {
            if ((status = submit_command(server_fd, &cmd)) == RAFT_COMMAND_APPLIED) {
                ...
                // The server lost and then regained leadership before we had the
                // chance to submit a request.
                assert(status == RAFT_BECAME_LEADER);
                break;
            }
...
{
    fprintf(stderr,
            "Usage: driver [-R] -i SERVER_ID -c CONFIG_PATH [-p PERSIST_PATH] [-s "
            "RANDOM_SEED]\n"
            "Servers are numbered according to the order in the config file,\n"
            "starting at 1.\n"
            "In recovery mode (-R specified), an existing persist file must be\n"
            "specified with -p.\n"
            "In normal mode (-R not specified), a persist path may be specified;\n"
            "if so, it must not refer to an existing file.\n"
            "The server process prints a (very noisy) log of sent and received\n"
            "messages to standard error, and a (much quieter) history of applied\n"
